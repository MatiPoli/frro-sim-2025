...
def testear_distribucion(nombre_dist, generador_func, params_dist,
                         ...
                         rango_grafica_teorica=None, usa_rechazo=False): # Nota: teor_media_func y teor_var_func han sido eliminados de los parámetros
    print(f"\n--- Graficando Distribución: {nombre_dist} con parámetros {params_dist} {'(Rechazo)' if usa_rechazo else '(T.Inversa)'} ---")

    muestras = []
    fallos_generacion_muestra = 0 # Para controlar si la generación falla demasiado

    for i in range(N_muestras):
        muestra_generada = None
        try:
            # La lógica de intentos para el método de rechazo está DENTRO de cada generador_func
            # Aquí solo llamamos a la función para obtener una muestra.
            muestra_generada = generador_func(*params_dist)
            if muestra_generada is not None: # Asegurarse de que se generó algo (importante si el generador puede fallar)
                muestras.append(muestra_generada)
            else: # Esto podría pasar si un generador devuelve None en algún caso extremo no manejado
                fallos_generacion_muestra += 1
        except NotImplementedError as nie:
            print(f"OMITIDO Gráfico {nombre_dist}: {nie}")
            return
        except Exception as e:
            # print(f"Error generando muestra {i+1}/{N_muestras} para {nombre_dist}: {e}. Se omitirá esta muestra.")
            fallos_generacion_muestra +=1
            # Si demasiadas muestras individuales fallan, abortamos para no seguir intentando inútilmente
            if fallos_generacion_muestra > N_muestras / 10 and N_muestras > 100 :
                ...
                return
            continue # Saltar a la siguiente muestra si esta falló

    if not muestras:
        print(f"No se generaron muestras válidas para {nombre_dist}. No se puede graficar.")
        return

    # --- SE HAN ELIMINADO LOS CÁLCULOS DE MEDIA, VARIANZA Y TASA DE ACEPTACIÓN ---

    plt.figure(figsize=(10, 6))
    # La lógica de ploteo permanece igual a la original
    if es_discreta:
...
if __name__ == "__main__":
    ...
    print("\n--- Iniciando graficación para distribuciones con MÉTODO DE RECHAZO ---") # Cambiado el mensaje
    ...
    testear_distribucion("Binomial", generar_binomial_rechazo, (n_bin, p_bin),
        # Se eliminaron: lambda n,p: n*p, lambda n,p: n*p*(1-p),
    ...
    testear_distribucion("Poisson", generar_poisson_rechazo, (lam_pois,),
        # Se eliminaron: lambda l:l, lambda l:l,
    ...
    print("\n--- Todas las gráficas completadas. Revisa las ventanas emergentes. ---") # Cambiado el mensaje